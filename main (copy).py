...
async def show_category(callback: types.CallbackQuery):
    ...
    for item, price in items.items():
        kb.button(text=f"{item} - {price} сом", callback_data=f"add_{category_key}_{item}") # Include category in callback for easier price lookup
...
async def add_to_cart(callback: types.CallbackQuery):
    ...
    try:
        _, category_key, item_name = callback.data.split("_", 2)
    except ValueError:
...
async def process_manual_address(message: Message, state: FSMContext):
    ...
    await state.update_data(address=address, address_type="manual")
    kb = InlineKeyboardBuilder()
    kb.button(text="✅ Да, верно", callback_data="confirm_address_yes")
...
async def process_phone_and_complete_order(message: Message, state: FSMContext):
    ...
    try:
        ...
        await bot.send_message(COURIERS_CHAT_ID, admin_notification_text) # Send to couriers chat
...
async def menu_help_contact(message: Message): # Renamed
    ...
    kb.button(
        ...
        url="https://t.me/DilovarAhi" # Make sure this is the correct contact
    )
    ...
    await message.answer(
        ...
        "• <b>Telegram:</b> @DilovarAhi (нажмите кнопку ниже)\n"
        "• <b>Телефон:</b> +992 98 765 43 21 (звонки/мессенджеры)\n\n" # Example number
